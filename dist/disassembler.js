...
function disassemble(op, cpu) {
    const opcodes = [
        // 0x00 - 0x0F
        "NOP", "LD BC, {d16}", "LD (BC), A", "INC BC", "INC B", "DEC B", "LD B, {d8}", "RCLA", "LD ({a16}), SP", "ADD HL, BC", "LD A, (BC)", "DEC BC", "INC C", "DEC C", "LD C, {d8}", "RRCA",
        // 0x10 - 0x1F
    ...
    */
    ...
    if (instr.includes("{d8}")) {
        ...
        instr = instr.replace("{a8}", utils_1.displayAsHex(addr));
    }
    else if (instr.includes("{a16}")) {
        const addr = cpu.readTwoByteValue(cpu.PC + 1);
        instr = instr.replace("{a16}", utils_1.displayAsHex(addr));
    }
